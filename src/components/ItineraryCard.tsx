    ...
    <div key={item.name} className="bg-white rounded-lg p-4 shadow-sm border">
      <div className="flex items-start gap-3">
        ...
        {item.imageUrl && (
          <img
            src={item.imageUrl}
            alt={item.imageAlt || `Image of ${item.name}`} // Use imageAlt or fallback
            className="w-24 h-24 object-cover rounded-md flex-shrink-0" // Adjust size as needed
          />
        )}
        <div className="flex-1">
          ...
          {/* No imageCredit rendering here, as per your request */}
        </div>
      </div>
    </div>
  );
